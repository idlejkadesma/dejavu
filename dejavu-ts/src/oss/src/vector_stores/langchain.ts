...
import { VectorStore } from "./base"; // dejavu's VectorStore interface
...
export class LangchainVectorStore implements VectorStore {
  ...
  async insert(
    ...
    const documents = payloads.map((payload, i) => {
      // Provide empty pageContent, store dejavu id and other data in metadata
      return new Document({
        pageContent: "", // Add required empty pageContent
        metadata: { ...payload, _dejavu_id: ids[i] },
      });
    });

    // Use addVectors. Note: Langchain stores often generate their own internal IDs.
    // We store the dejavu ID in the metadata (`_dejavu_id`).
    try {
      await this.lcStore.addVectors(vectors, documents, { ids }); // Pass dejavu ids if the store supports it
    } catch (e) {
  ...
  async search(
    ...
    // Filters passed via dejavu interface are not reliably translatable to generic Langchain stores.
    ...
    // Map Langchain results [Document, score] back to dejavu VectorStoreResult
    return results.map(([doc, score]) => ({
      id: doc.metadata._dejavu_id || "unknown_id",
  ...
  async get(vectorId: string): Promise<VectorStoreResult | null> {
    ...
    // Perform a search with a filter like { _dejavu_id: vectorId }, limit 1.
    // This requires the underlying store to support filtering on _dejavu_id.
  }
  ...
  async update(
    ...
    // Possible implementation: Check if store has delete, call delete({_dejavu_id: vectorId}), then insert.
  }
  ...
  async delete(vectorId: string): Promise<void> {
    ...
    if (typeof (this.lcStore as any).delete === "function") {
      try {
        // We need to delete based on our stored _dejavu_id.
        // Langchain's delete often takes its own internal IDs or filter.
        // Attempting deletion via filter is the most likely approach.
        console.warn(
          "LangchainVectorStore: Attempting delete via filter on '_dejavu_id'. Success depends on the specific Langchain VectorStore's delete implementation.",
        );
        await (this.lcStore as any).delete({ filter: { _dejavu_id: vectorId } });
