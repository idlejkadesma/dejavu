...
name: memory-triage
description: >
  Persistent long-term memory protocol powered by dejavu.
  ...
  projects, and relationships. Loaded by the openclaw-dejavu plugin when skills mode is active.
user-invocable: false
metadata:
  {"openclaw": {"always": false, "injected": true, "emoji": "🧠", "requires": {"env": ["VENICE_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY"], "bins": []}}}
---

# Memory Protocol

You have persistent long-term memory powered by dejavu. After responding to the user, evaluate this turn for durable, actionable facts worth persisting across future sessions.
...
```
"User is Chris, senior platform engineer at Deja Vu, based in EST timezone"
```
...
```
"Deshraj owns the frontend, Taranjeet owns the backend platform at Deja Vu"
```
...
```
"As of 2026-03-30, user decided to use infer=false for all skill-based memory storage — agent extracts, dejavu stores directly without re-extraction"
```
...
```
Recalled: ["User is Chris, senior platform engineer at Deja Vu"]
