---
title: Quickstart
description: "Set up your Deja Vu Platform account, install the SDK, and store your first memory in under five minutes."
icon: "bolt"
iconType: "solid"
---

Get started with Deja Vu Platform's hosted API in under 5 minutes. This guide shows you how to authenticate and store your first memory.
...
- Deja Vu Platform account (<a href="https://app.dejavu.ai?utm_source=oss&utm_medium=platform-quickstart" rel="nofollow">Sign up here</a>)
- API key (<a href="https://app.dejavu.ai/dashboard/settings?tab=api-keys&subtab=configuration" rel="nofollow">Get one from dashboard</a>)
...
```bash CLI
dejavu init --api-key "your-api-key"
```
...
```bash CLI
dejavu add "I'm a vegetarian and allergic to nuts." --user-id user123
```
...
```bash CLI
dejavu search "What are my dietary restrictions?" --user-id user123
```
...
<Callout type="tip" icon="plug">
  **Pro Tip**: Want AI agents to manage their own memory automatically? Use <Link href="/platform/dejavu-mcp">Deja Vu MCP</Link> to let LLMs decide when to save, search, and update memories.
</Callout>
...
- **[Integration Examples](/cookbooks/companions/quickstart-demo)** - See Deja Vu in action
