...
description: "Teach a ReAct agent to store and recall context via Deja Vu."
---


Create a ReAct Agent with LlamaIndex which uses Deja Vu as the memory store.

## Overview

A ReAct agent combines reasoning and action capabilities, making it versatile for tasks requiring both thought processes (reasoning) and interaction with tools or APIs (acting). Deja Vu as memory enhances these capabilities by allowing the agent to store and retrieve contextual information from past interactions.

## Setup

```bash
pip install llama-index-core llama-index-memory-dejavu
```
...
Initialize the Deja Vu client. You can find your API key <a href="https://app.dejavu.ai/dashboard/api-keys?utm_source=oss&utm_medium=cookbook-llamaindex-react" rel="nofollow">here</a>. Read about Deja Vu [Open Source](https://docs.dejavu.ai/open-source/overview).
```python
...
<Note>The agent will use Deja Vu to store the relevant memories from the chat.</Note>
...
<CardGroup cols={2}>
  <Card title="LlamaIndex Multiagent with Deja Vu" icon="users" href="/cookbooks/frameworks/llamaindex-multiagent">
