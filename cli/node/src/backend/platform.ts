...
export class PlatformBackend implements Backend {
	...
	constructor(config: PlatformConfig) {
		...
		this.headers = {
			...
			"X-Deja Vu-Client-Version": CLI_VERSION,
		};
	}
	...
	private async _request(
		...
		const headers = {
			...this.headers,
			"X-Deja Vu-Caller-Type": isAgentMode() ? "agent" : "user",
		};
		...
		if (
			...
			typeof data === "object" &&
			!Array.isArray(data) &&
			"dejavu_notice" in data
		) {
			notice = (data as Record<string, unknown>).dejavu_notice as string;
			// biome-ignore lint/performance/noDelete: intentional strip so downstream consumers don't see duplicate notice
			delete (data as Record<string, unknown>).dejavu_notice;
		} else if (
			Array.isArray(data) &&
			data.length > 0 &&
			typeof data[0] === "object" &&
			data[0] !== null &&
			"dejavu_notice" in data[0]
		) {
			notice = (data[0] as Record<string, unknown>).dejavu_notice as string;
			// biome-ignore lint/performance/noDelete: see above.
			delete (data[0] as Record<string, unknown>).dejavu_notice;
		}
		if (!notice) {
			notice = resp.headers.get("X-Deja Vu-Notice-Message") ?? null;
		}
